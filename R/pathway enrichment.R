...
available_protein_and_ks_datasets <- function(){
  print(names(protools3::protein_and_ks_sets))
}
...
ksea <- function(df.fold,ks_db=c("pdts","psite"), graph.heading="",pval.cut.off=0.05){
  ...
  yy <- protools3::expand.phosphopeptide.dataset(df.fold)
  cores=detectCores()
  cl <- makeCluster(cores[1]-1)
  registerDoParallel(cl)
  xx <- foreach(db = ks_db,.combine = "rbind")%do%{
    x <- protools3::kinase.substrate.enrichment(dfx= yy,ks_db =  db)
    x
  }
  stopCluster(cl)
...
kinase.substrate.enrichment <- function(dfx, ks_db,is.ksea=TRUE){
  ...
  dfx[,1] <- gsub(".","(",dfx[,1],fixed = T)
  nc <- ncol(dfx)
  df.ks <- protools3::protein_and_ks_sets[[ks_db]]
  nr <- nrow(df.ks)
...
pathway_enrichment_matrix <- function(list_of_comparisons_from_limma,
  ...
  for (comp in list_of_comparisons_from_limma){
    ...
    if (length(decreased.peptides)>0 & length(increased.peptides)>0){
      pe <- protools3::pathway_enrichment(increased.peptides = increased.peptides,
...
pathway_enrichment_matrix_by_page_and_ks <- function(list_of_comparisons_from_limma,
  ...
  for (comp in list_of_comparisons_from_limma){
    ...
    xx <- foreach(db = prot_dbs,.combine = "rbind")%dopar%{
      x <- protools3::kinase.substrate.enrichment(dfx= dd[[1]],ks_db =  db, is.ksea=FALSE)
      x
    }
    stopCluster(cl)
...
pathway_enrichment <- function(increased.peptides,
  ...
  enrich.up <- foreach(db = prot_dbs, .combine = "rbind")%dopar%{
    ...
    e <- protools3::enrichment.from.list(list.of.peptides=increased.peptides,
                                        background.list,
                                        prot_db = db,
                                        is.ksea = is.ksea)
    if (nrow(e)>0){
  ...
  enrich.do <- foreach(db = prot_dbs, .combine = "rbind")%dopar%{
    ...
    e <- protools3::enrichment.from.list(list.of.peptides =  decreased.peptides,
                                         background.list,
  ...
  if (length(enrich.do)>0 & length(enrich.up)>0){
    ...
    enrich.up <- subset(enrich.up,enrich.up$counts>n_counts_cutoff)
    enrich.do <- subset(enrich.do,enrich.do$counts>n_counts_cutoff)
    ...
    diff.enrich <- diff.enrich[order(-diff.enrich$delta.counts),]

    dif.e.up <- diff.enrich[1:25,]

    diff.enrich <- diff.enrich[order(diff.enrich$delta.counts),]
    dif.e.do <- diff.enrich[1:25,]
    ...
    if (dothis==1){
      ...
      enrich.combined <- foreach(db = prot_dbs, .combine = "rbind")%dopar%{
        ...
        e <- protools3::enrichment.from.list(list.of.peptides=c(increased.peptides,decreased.peptides),
                                            background.list,
                                            prot_db = db,
                                            is.ksea = is.ksea)
        if (nrow(e)>0){
    ...
    plot.diff <- NA
    plot.pathways.enrichment.by.counts <- NA
    plot.pathways.enrichment.by.pvalue <- NA
    plot.pathways.enrichment.by.enrichment <- NA
    prots.in.decreased.pathways <- NA
    prots.in.increased.pathways <- NA
    ...
    if (make.plots==T){
      ...
      plot.diff <- ggplot(rbind.data.frame(dif.e.do,dif.e.up),aes(x=delta.enrichment.up.vs.down,y=reorder(pathway,
        ...
        geom_point(aes(size=abs(delta.counts),color=delta.pval.up.vs.down), outlier.colour="red")+
      ...
      plot.pathways.enrichment.by.pvalue <- ggplot(na.omit(enrich.up.do),aes(x= -log10(FDR) ,
                                                                                 y=reorder(pathway,-log10(FDR))))+
        geom_point(aes(size=counts,color=log2(enrichment)))+
        labs(title = "Pathway Enrichement Analysis", color="-log2(e)",
      ...
      uniprot.data <- protools3::uniprot.names
      ...
      plot.all <- ggplot(mypathways,aes(x=ontology,y=name.short))+
        scale_color_manual(values=protools3::mycolors()$C23 )+
  ...
  return(list(pathway_enrichment_data=rbind.data.frame(enrich.do,enrich.up),
              ...
              plot.pathways.enrichment.by.pvalue=plot.pathways.enrichment.by.pvalue,
...
enrichment.from.list <- function(
  ...
  #length(intersect(background.list,list.of.peptides))
 ...
 # }
  ...
  list.of.peptides <- unique(trimws (unlist(list.of.peptides)))
  background.list <- unique(trimws(  unlist(background.list)))
  ####################



  df.ks <- protools3::protein_and_ks_sets[[prot_db]]
  ...
  #length(setdiff(background.data,list.of.peptides))
  bg.size <- numeric(nr)
